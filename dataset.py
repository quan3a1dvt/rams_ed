...
class EDDataset(Dataset):
    ...
    def __getitem__(self, item):
        ...
        all_pieces = [self.CLS]
        transform_matrix[0, len(all_pieces) - 1] = 0.0
        all_spans = []
        ...
        for idx, word in enumerate(words):
            ...
            if len(pieces) != 0:
                ...
                transform_matrix[idx, start:end] = mean_matrix

        all_pieces.append(self.SEP)
        cls_text_sep_length = len(all_pieces)
        transform_matrix[len(words), cls_text_sep_length - 1] = 0.0
        ...
        assert len(all_pieces) == self.BML
        targets = targets + [-100] * (self.WML - len(targets))

        # print(len(targets), len(words))
        # assert len(targets) == len(words)
    ...
    def pack(items):
        ...
        # pack max length
        mini_pack['word_longest_length'] = torch.max(mini_pack['word_length'])
        mini_pack['bert_longest_length'] = torch.max(mini_pack['bert_length'])

        WL = mini_pack['word_longest_length'].item()
        # cut off target == word max length
        target = mini_pack['target']
        target = [x[:WL] for x in target]
        target = flatten(target)
        mini_pack['target'] = target
        return mini_pack
...
if __name__ == '__main__':
    ...
    if sublabel == 3:
        ...
        train = load_sentence_data('data-2/train.jsonlines')
        dev = load_sentence_data('data-2/dev.jsonlines')
        test = load_sentence_data('data-2/test.jsonlines')        
